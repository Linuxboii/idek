...
backend directory:  C:\Users\paran\OneDrive\Documents\Claude\Projects\n8n-python
---
...
## Task 2: Host app — Install dependency and add env vars

**Files (in host app):**
- Modify: `package.json` (via npm install)
- Create/modify: `.env` (or `.env.local`)
...
- [ ] **Step 2: Add env vars to `.env` (or `.env.local`)**
...
- Uses `VITE_CRM_API_BASE_URL` instead of `VITE_API_BASE_URL`
...
- Reads token from `crmState.js` instead of localStorage
- Uses `VITE_CRM_API_BASE_URL` for WebSocket base
...
export function useWebSocket(onMessage) {
  ...
  const connect = useCallback(() => {
    ...
    const base = CRM_BASE.replace(/^https/, 'wss').replace(/^http/, 'ws')
    const socket = new WebSocket(`${base}/ws?token=${crmToken}`)
